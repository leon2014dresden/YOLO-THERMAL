...
class Metric(SimpleClass):
    ...
    def p_IoU(self):
        """
        Returns the Average Precision (AP) of all classes for the IoU threshold from 0.5 to 0.95 with step 0.05.

        Returns:
            (np.ndarray, list): Array of shape (, 10) with all classes per IoU value, or an empty list if not available.
        """
        return np.round(self.all_ap.mean(0), 3) if len(self.all_ap) else []    
